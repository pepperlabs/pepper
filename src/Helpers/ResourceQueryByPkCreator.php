...
class ResourceQueryByPkCreator extends ResourceCreator
{
    protected $path;
    protected $stub;
    ...
    protected function updateConfig($name)
    {
        ...
        $name = Str::of($name)->snake() . '_by_pk';
        if ($this->configKeyExists('graphql.schemas.default.query.' . $name)) {
            $class = strval('App\GraphQL\Queries\Pepper\\' . $className . 'QueryByPk::class');
